...
const Home = () => {
  ...
  // Banner carousel
  const { currentIndex: bannerIndex, prevSlide: prevBanner, nextSlide: nextBanner } = useBannerCarousel(images.length, 5000);

  // Fetch movie data
  useEffect(() => {
    movieService.getNowPlaying(0, 20).then((res) => setNowPlaying(res.content || []));
    movieService.getUpcoming(0, 20).then((res) => setUpcoming(res.content || []));
  }, []);
  ...
  const renderMovieCard = (movie: MovieSummary) => (
    <div key={movie.id} className="group relative flex flex-col transition">
          ...
          <div className="absolute inset-0 bg-black bg-opacity-60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center items-center text-center p-4">
            <div className="text-white text-left">
              ...
              <p className="text-xs font-light mb-1 flex items-center">
                <MapPin size={16} className="mr-2 text-red-500" /> {formatGenres(movie.genres)}
              </p>
              <p className="text-xs font-light mb-1 flex items-center">
                <Clock size={16} className="mr-2 text-red-500" /> {movie.time}’
              </p>
              <p className="text-xs font-light mb-1 flex items-center">
                <Globe size={16} className="mr-2 text-red-500" /> {formatSpokenLanguages(movie.spokenLanguages)}
              </p>
              <p className="text-xs font-light flex items-center">
                <ShieldAlert size={16} className="mr-2 text-red-500" /> {movie.age}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className={`flex w-full mt-2 space-x-2 ${movie.trailer ? "justify-start" : "justify-center"}`}>
  ...
  const renderCarousel = (carousel: ReturnType<typeof useCarousel>, data: MovieSummary[]) => (
      ...
      <div className="flex justify-center mt-3 space-x-2">
        {Array.from({ length: carousel.totalSlides }).map((_, idx) => (
          <button
            ...
            className={`w-3 h-3 rounded-full ${idx === carousel.currentIndex ? "bg-white" : "bg-gray-500"}`}
          />
        ))}
      </div>
    </div>
      ...
      <div className="w-full min-h-screen pb-16">
        ...
        <div className="relative w-full">
          <section className="w-full max-w-5xl mx-auto aspect-[16/6] overflow-hidden rounded-sm shadow-lg">
            ...
            >
              {images.map((src, idx) => (
                <img key={idx} src={src} alt={`Slide ${idx}`} className="min-w-full h-full object-cover" />
              ))}
            </div>
          </section>
          <button onClick={prevBanner} className="absolute left-28 top-1/2 -translate-y-1/2 p-2 z-10 text-white hidden sm:block focus:outline-none">
            <ChevronLeft size={40} />
          </button>
          <button onClick={nextBanner} className="absolute right-28 top-1/2 -translate-y-1/2 p-2 z-10 text-white hidden sm:block focus:outline-none">
        ...
        <section className="relative w-full max-w-6xl mx-auto mt-12">
          ...
          <div className="flex justify-center mt-5">
            <Link to="/movies/now-playing" className="bg-red-600 text-white px-6 py-2 rounded-lg font-semibold hover:bg-red-700 transition">
              Xem thêm
            </Link>
          </div>
        </section>
        ...
        <section className="relative w-full max-w-6xl mx-auto mt-16">
          ...
          <div className="flex justify-center mt-5">
            <Link to="/movies/upcoming" className="bg-red-600 text-white px-6 py-2 rounded-lg font-semibold hover:bg-red-700 transition">
              Xem thêm
            </Link>
          </div>
        </section>
